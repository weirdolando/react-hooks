...
component. That's no problem. Separate state logically by domain. If it
**changes together**, it's likely better to keep together in a reducer. If
something is pretty independent from other elements of state in that
hook/component, then putting it with other elements of state is just adding
unnecessary complexity and noise to that reducer and you'd be better off leaving
it out on its own.

### Error Boundary

When there's an error, React will find the nearest component that has
`static getDerivedStateFromError` method.

Error boundaries do not catch errors for:

- Event handlers (If you need to catch an error inside an event handler, use the
  regular JavaScript `try` / `catch` statement. Learn
  [more](https://reactjs.org/docs/error-boundaries.html#how-about-event-handlers))
- Asynchronous code (e.g. `setTimeout` or `requestAnimationFrame` callbacks,
  promises)
- Server side rendering
- Errors thrown in the error boundary itself (rather than its children)

If you want to handle asynchronous code as well, you can use a library called
_react-error-boundary_ that has a `useErrorHandler` hooks.
